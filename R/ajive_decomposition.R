...
#'
#' @param blocks A list of numeric matrices with same number \eqn{N} of rows
#'   specifying the data blocks to be analyzed.
#' @param initial_signal_ranks An integer vector specifying the initial ranks
#'   for the signal matrices.
#' @param scale_blocks A boolean specifying whether blocks should be standardized. 
#'   Defaults to `FALSE`.
...
ajive <- function(blocks,
  ...
  if (scale_blocks) {
    blocks <- lapply(blocks, base::scale)  
  }
  ...
  if (is.null(joint_scores)) {
    ...
    if (is.null(dim(joint_scores))) {
      joint_scores <- matrix(joint_scores, ncol = 1)
    }
  ...
  list(
    ...
    joint_scores = joint_scores,
    joint_rank = joint_rank,
    joint_rank_sel_results = joint_rank_sel_results
  )
}

#' The singular value threshold.
#'
#' Computes the singular value threshold for the data matrix (half way between
#' the rank and rank + 1 singular value).
#'
#' @param singular_values A numeric vector specifying the singular values.
#' @param rank An integer value specifying the rank of the approximation.
get_sv_threshold <- function(singular_values, rank) {
  rank_max <- length(singular_values)
  if (rank == rank_max) {
    return(0.95 * singular_values[rank])
  }
...
#'
#' @inheritParams ajive
#' @param block_svd A list of lists specifying the SVDs of each data block.
#' @param sv_thresholds A numeric vector specifying the singular value
#'   thresholds from the initial signal rank estimates.
#'
#' @return A numeric matrix of size \eqn{N \times r_J} storing the joint scores.
get_joint_scores <- function(blocks,
  ...
  if (is.na(n_wedin_samples) &&
    is.na(n_rand_dir_samples) &&
    is.na(joint_rank)) {
    cli::cli_abort("At least one of `n_wedin_samples`, `n_rand_dir_samples`,
  ...
  if (is.na(joint_rank)) {
    ...
    if (!is.na(n_rand_dir_samples)) {
      rand_dir_samples <- get_random_direction_bound(
        n_obs = n_obs,
    ...
    overall_sv_sq_threshold <- max(
      wedin_svsq_threshold,
    ...
    for (k in 1:K) {
      for (j in 1:joint_rank_estimate) {
        ...
        if (sv < sv_thresholds[[k]]) {
          cli::cli_alert_info("Removing column {j}")
...
#' @param joint_scores Matrix. The basis of the joint space (dimension n x
#'   joint_rank).
#' @param sv_threshold Numeric vector. The singular value thresholds from the
#'   initial signal rank estimates.
#' @param full Boolean. Do we compute the full J, I matrices or just the SVDs
